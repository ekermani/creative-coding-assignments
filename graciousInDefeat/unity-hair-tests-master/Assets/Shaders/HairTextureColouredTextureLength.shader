	...
	SubShader{
		...
		Pass{
			...
			void AddEdgeVertexToTriangleStream(appdata_hair_gs hairVertex1, appdata_hair_gs hairVertex2, float3 tangent, inout TriangleStream<v2f> triangleStream) {
				...
				float4 lengthScale = tex2Dlod(_LengthMap, float4(hairuv.xy, 0, 0));

				float hairLength = _Length * lengthScale.x;
				BuildSprite(position, _Width, hairLength , direction, tangent, hairuv, triangleStream);
			}
